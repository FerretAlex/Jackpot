...
app.post("/api/register",async(req,res)=>{
  ...
  if(!email||!password)return res.json({error:"Email and password needed"});
  if(db.users.find(u=>u.email===email))return res.json({error:"User with such email already exists"});
...
app.post("/api/update-profile",auth,(req,res)=>{
  ...
  if(age!==undefined){
    ...
    if(n!==null&&(!Number.isFinite(n)||n<0||n>120))return res.status(400).json({error:"Invalid age"});
    user.age=n;
  }

  if(gender!==undefined)user.gender=gender||null;
  if(faculty!==undefined)user.faculty=faculty||null;

  if(course!==undefined){
    const n=(course===null||course==="")?null:Number(course);
    if(n!==null&&(!Number.isFinite(n)||n<1||n>7))return res.status(400).json({error:"Invalid course"});
    user.course=n;
  }
...
app.post("/api/swipe",auth,(req,res)=>{
  ...
  if(swipeType==="like"){
    const reverseLike=db.swipes.find(s=>s.from===toUserId&&s.to===fromUserId&&s.type==="like");
    if(reverseLike){
      ...
      if(!match){
        ...
        saveDB(db);
        return res.json({status:"match",matchId:match.id});
      }

      saveDB(db);
      return res.json({status:"already_matched",matchId:match.id});
    }
  }
...
app.get("/api/matches",auth,(req,res)=>{
  ...
  const userMatches=db.matches
    .filter(m=>Number(m.user1)===userId||Number(m.user2)===userId)
    .map(m=>{
